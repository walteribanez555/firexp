...
import { bedrockClient, BEDROCK_MODEL_ID, PROMPT_MODE } from './prompts.config';
import type {
...
function buildSystemPrompt(kb: KnowledgeBase, chapterId: string, kbDecision: KbDecision): string {
  ...
  const { target } = { target: kbDecision.nudgeTarget };
...
async function generateWithAI(
  ...
  const response = await bedrockClient.send(new ConverseCommand({
    ...
    inferenceConfig: {
      maxTokens:   256,
      temperature: 0.8,   // some creativity for prompt variation
    },
  }));
...
export const promptsService = {
  async generate(req: PromptRequest, staticPrompt?: string): Promise<GeneratedPrompt> {
    ...
    if (PROMPT_MODE === 'random') {
      return generateRandom(kbDecision, req.options, req.flags, staticPrompt);
    }

    // AI mode — with fallback to random on error
    try {
      return await generateWithAI(kb, req.chapterId, kbDecision, req.options, req.flags);
    } catch (err) {
