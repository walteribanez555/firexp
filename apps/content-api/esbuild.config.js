...
// Lambda bundles src/index.ts → dist/index.js (CJS, no @hono/node-server)
// ECS / default bundles src/main.ts → dist/main.js
const isLambda = target === 'lambda';

const config = {
  entryPoints: [isLambda ? 'src/index.ts' : 'src/main.ts'],
  ...
  // ECS/local will also run on >=20 so node22 is safe for both.
