...
| **infra/cdk** — DynamoDB (series/episodes/sessions) + S3 + CloudFront(OAC) + Lambda + HTTP API | ✅ (no desplegado) |
| **infra/terraform** — IAM Bedrock + prompt-generator | ✅ (no desplegado) |
...
- **No desplegado en AWS**: todo corre local (DynamoDB Local, relay, content‑api). El deploy (CDK/Terraform) está listo pero requiere tu OK y habilitar acceso a modelos Bedrock en consola.
- **relay = local** (WebSocket); el paso a **Fargate** está planificado, no hecho.
